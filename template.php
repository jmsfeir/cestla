	...
	<div id="give-form-<?php echo $form->ID; ?>-wrap" class="<?php echo $form_wrap_classes; ?>">
		...
		<?php if ( $form->is_close_donation_form() ) {
			...
			<form id="give-form-<?php echo $form_id; ?>" class="<?php echo $form_classes; ?>"
                ...
                <?php if (is_page('38') || is_page('45') || is_page('104') || is_page('106') || is_page('107') || is_page('108') || is_page('109')) {
  echo pll_e('Your donation is tax deductible in all EU member states.');
  echo'<a style=" text-decoration: underline; color:white" href="http://download.eurordis.org.s3.amazonaws.com/donation/eurordis-tax-deduction.pdf" target="_blank"> ';
  echo pll_e('Find out more.') .'</a>.';
  }
	?>
...
function give_output_levels( $form_id ) {
	...
	switch ( $display_style ) {
		case 'buttons':
			...
			$output .= '</ul>';
			$output .= '<div class="give-total-wrap"><div class="give-donation-amount form-row-wide"><input class="give-text-input give-amount-top" id="give-amount" name="give-amount" type="tel"
				       placeholder="';	   
			$output .= $custom_amount_text;	
			$output .= '" value autocomplete="off"><span class="give-currency-symbol give-currency-position-after">€</span></div></div>';
...
function give_user_info_fields( $form_id ) {
	...
	<fieldset id="give_checkout_user_info">
		<legend> <?php pll_e('Personal information'); ?></legend>
     
		<p id="give-first-name-wrap" class="form-row form-row-first">
			<label class="give-label" for="give-first">
				<?php pll_e('First name'); ?>
			...
			<input
				...
				type="text"
				name="give_first"
				placeholder="<?php pll_e('First name'); ?>"
				id="give-first"
		...
		<p id="give-last-name-wrap" class="form-row form-row-last">
			...
			<input
				...
				name="give_last"
				id="give-last"
				placeholder="<?php pll_e('Last name'); ?>"
		...
		<p id="give-email-wrap" class="form-row form-row-wide">
			<label class="give-label" for="give-email">
				<?php esc_html_e( pll_e('Email Address'), 'give' ); ?>
			...
			<input
				...
				type="email"
				name="give_email"
				placeholder="<?php esc_html_e( pll_e('Email Address'), 'give' ); ?>"
				id="give-email"
	...
	<legend><?php echo apply_filters( 'give_credit_card_fieldset_heading', esc_html__( 'Credit Card Info', 'give' ) ); ?></legend>
		...
		<p id="give-card-number-wrap-<?php echo $form_id ?>" class="form-row form-row-two-thirds">
			<label for="card_number-<?php echo $form_id ?>" class="give-label">
				<?php esc_html_e( 'Card N0umber', 'give' ); ?>
				<span class="give-required-indicator">*</span>
				<span class="give-tooltip give-icon give-icon-question"
			...
			<input type="tel" autocomplete="off" name="card_number" id="card_number-<?php echo $form_id ?>"
			       class="card-number give-input required" placeholder="<?php esc_attr_e( 'Card n0umber', 'give' ); ?>"
...
function give_payment_mode_select( $form_id ) {
	...
	<fieldset id="give-payment-mode-select" <?php if ( count( $gateways ) <= 1 ) {
		...
		?>
		<legend
			class="give-payment-mode-label"><?php pll_e('Select payment method'); ?></legend>
				...
				foreach ( $gateways as $gateway_id => $gateway ) :
					...
					<li<?php echo $checked_class?>>
						...
						<label for="give-gateway-<?php echo esc_attr( $gateway_id ) . '-' . $form_id; ?>"
						       ...
						       id="give-gateway-option-<?php echo esc_attr( $gateway_id ); ?>"> <?php echo pll_e(esc_html( $gateway['checkout_label'] )); ?></label>
					</li>
					<?php
				endforeach;
				?>
...
function give_checkout_final_total( $form_id ) {
	...
	<p id="give-final-total-wrap" class="form-wrap ">
		<span
        	...
        	class="give-donation-total-label"> <?php if (is_page('294')) {
    echo 'Total amount:';}
	else { echo apply_filters( 'give_donation_total_label', esc_html__( pll_e('Donation Total:'), 'give' ) );}
	?></span>
